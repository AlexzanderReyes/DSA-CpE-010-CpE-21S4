...
int main() {
    ...
    // Task 3.4: Add new node 'O' as a child of node 'G'
    TreeNode* O = new TreeNode('O');
    addChild(G, O);

    // Perform the findData function for the new node 'O'
    cout << "Finding 'O' using pre-order:" << endl;
    findData(A, "pre", 'O');

    cout << "Finding 'O' using post-order:" << endl;
    findData(A, "post", 'O');

    cout << "Finding 'O' using in-order (assuming binary structure):" << endl;
    findData(A, "in", 'O');
